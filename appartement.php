...
$nbre_media = $bdd->query("SELECT count(*) AS nb FROM annonces WHERE type = 'Appartement'");
...
?>
	...
	<?php
		$sql = $bdd->query("SELECT * FROM annonces WHERE type = 'Appartement' ORDER BY id_a DESC LIMIT $page_num,$media_par_Page");
					...
					<?php 
						for($i = 1; $i<= $nbres_pages; $i++) {
								if($_GET['page'] == $i){
									...
									echo "<a class=\"ch_page\" href='appartement.php?page=".$i."'>".$i."</a>";
								}
						}
					?>
			</div>
