...
$nbre_media = $bdd->query("SELECT count(*) AS nb FROM annonces WHERE type = 'Autre'");
...
?>
	...
	<?php
		$sql = $bdd->query("SELECT * FROM annonces WHERE type = 'Autre' ORDER BY id_a DESC LIMIT $page_num,$media_par_Page");
	...
	?>
		<div class="article">
            ...
            <a href="image/<?= $id ?>-2.jpg" class="zoombox <?= $gallery ?>"><img src="image/<?= $id ?>-3.jpg" alt="" class="" style="display : none"></a>
					...
					<?php 
						for($i = 1; $i<= $nbres_pages; $i++) {
								if($_GET['page'] == $i){
									...
									echo "<a class=\"ch_page\" href='autre.php?page=".$i."'>".$i."</a>";
								}
						}
					?>
			</div>
